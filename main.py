...
current_dirrectory = Path(__file__).parent # Par exemple : 'user/Document/.../2A/GIE/OREXA/projet_web' (marche sur windows et mac)
BDD_PATH = str(current_dirrectory / 'BDD.db') # 'user/Document/.../2A/GIE/OREXA/projet_web/BDD.db' 
...
def get_table_next_id(table:str):
    ...
    id_ = id_ + 1 if id_ is not None else 0 # verifie si la table est vide (dans ce cas id_=None) (sinon id_ = 4 par exemple(int type))
    return id_
...
# Common function ---------------------------------------------------
def stocks(sql, table_name, html_name, type):
    stocks = execute_sql(sql)
    
    if request.method != 'POST':
        return render_template(html_name, stocks=stocks, msg = '') # re-initilise les input html avec un champs vide
    else:
        id_type = request.form.get(f'id_{type}', '')
        ...
        if condition_1 and condition_2 :
            ...
            id_type, nombre, mode = int(id_type), int(nombre), int(mode) # else n'est pas utile (visuelle)
        
        table_name_id = execute_sql(f'SELECT id_{type} FROM {table_name}')
        id_type_list = [ligne[f'id_{type}'] for ligne in table_name_id]
        if id_type not in id_type_list:
            return render_template(html_name, stocks=stocks, msg='id non valide')  
       

        quantiter = execute_sql(f'SELECT quantiter FROM {table_name} WHERE id_{type}=?', (id_type,))[0]['quantiter']    
        ...
        sql_update = f"UPDATE {table_name} SET quantiter=? WHERE id_{type}=?"
...
def maj_stock(id_commande, mode, agi_lean_ou_log:str):
    ...
    type = 'chassis' if agi_lean_ou_log=='agilean' else 'piece'
    id_ = 'id' if agi_lean_ou_log=='agilean' else 'id_commande' 
    
    sql = f'''SELECT stock.quantiter, stock.id_{type} 
    FROM {table_commande} AS cmd
    JOIN {table_stock} AS stock ON cmd.id_{type}=stock.id_{type} 
    WHERE cmd.{id_}=?'''
    table = execute_sql(sql, (id_commande,) )   
    for ligne in table:
            
        quantiter = ligne["quantiter"]
        id_type = ligne[f"id_{type}"]
        if mode == '+':
            quantiter += 1
        elif mode == '-':
            quantiter -= 1
        ...
        sql_update = f"UPDATE {table_stock} SET quantiter=? WHERE id_{type}=?"
        commit_sql(sql_update, values)
...
def create_app(): # pas obligatoire mais peux etre utile pour test le site (pytest par exemple), evite de verifier si tout fonction quand on fait des modifs
    ...
    def stock_agilean(): 
        ...
        type = 'chassis'
        return stocks(sql, table_name, html_page, type)
    ...
    def pieces_detachees_agilean():
        ...
        if request.method != 'POST':
            return render_template('pieces_detachees_agilean.html', liste_pieces=liste_pieces, msg='')
        ...
        for id_piece in pieces:
            ...
            if not n.isdigit(): # verifie le formulaire 
                return render_template('pieces_detachees_agilean.html', liste_pieces=liste_pieces, msg='Mauvaise selection') 
    ...
    def suivi_commande_agilean():
        ...
        etat_commande = request.form.get('etat_commande', '') # Renvoi l'etat de la commande
        id_commande = request.form.get('id_commande', '') # Renvoi l'id de la commande
        ...
        envoi_agilog = request.form.get('envoi_agilog', '') # Renvoi l'id de la commande
        ...
        reception_agilog = request.form.get('reception_agilog', '') # Renvoi l'id de la commande
        if reception_agilog != '':
    ...
    def sent_command_to_agilog_maj_bdd(id_commande):
        table = execute_sql('SELECT id_chassis, id_option FROM commande_agilean WHERE id=?', (id_commande,))
    ...
    def suivi_commande_agilog_to_agigreen():
        ...
        commandes = execute_sql(sql)
        if request.method != 'POST' :
            return render_template('suivi_commande_agilog_to_agigreen.html', commandes=commandes)
    ...
    def suivi_commande_agilog_from_agilean():
        ...
        if request.method != 'POST':
            return render_template('suivi_commande_agilog_from_agilean.html', commandes=commandes)
        
        sent_id_commande_to_agigreen = request.form.get('sent_id_commande_to_agigreen', '')
        if sent_id_commande_to_agigreen !='':
            ...
            commit_sql(sql, (get_current_time(), 'commande envoyée a agigreen', id_commande))
    ...
    def sent_commande_to_agigreen_maj_bdd(id_commande):
        commandes = execute_sql('SELECT id_piece, quantiter FROM commande_agilog WHERE id_commande=?', (id_commande,))
    ...
    def preparer_commande_agigreen():
        ...
        return render_template('commande_agigreen.html', commandes=commandes, id_commande=id_commande)
    ...
    def commande_client():
        ...
        condition = (nom == '') or (prenom == '') or (id_chassis == '') or (options == [])
        if condition : 
        ...
        id_option = ("0"+options if len(options)==2 else "00"+options) if len(options)<3 else options
    ...
    def supprimer_commande_client():
        ...
        table_nom_prenom = execute_sql('SELECT nom, prenom FROM commande_agilean WHERE id=?', (id_commande,))
    ...
    def afficher_commande_client():
        ...
        if id_commande != '':
            ...
            if not id_commande.isdigit() or id_commande not in [str(id_["id"]) for id_ in list_id]:
                return render_template('suivi_commande_client_id.html', msg='Mauvaise saisie') 
            values = (id_commande,)
        else:
            nom, prenom = request.form.get('nom', ''), request.form.get('prenom', '')
            values = (nom, prenom)
        commandes = get_table_commande_client(values=values) # commande client par nom et prenom
        return render_template('suivi_commande_client.html', commandes=commandes)
